...
public class ImageController {
    ...
    @PostMapping("/journal")
    public ResponseEntity<List<Image>> createJournalEntryImages(@RequestParam("files") List<MultipartFile> images,
                                                                @RequestParam("journalId") long journalEntryId
    ) {
        return new ResponseEntity<>(imageService.createJournalEntryImages(images, journalEntryId), HttpStatus.CREATED);
    }

    @GetMapping("/journal/{journalId}")
    public ResponseEntity<List<Image>> getImagesForJournalEntry(@PathVariable long journalId) {
